...
export function createSendEmailWorker() {
  return new Worker<SendEmailJobData>(
    ...
    async (job) => {
      const db = getDb();
      const trackerUrl = process.env.TRACKER_URL ?? "http://localhost:3003";
      ...
      const [workspace] = await db.select().from(workspaces).where(eq(workspaces.id, send.workspaceId)).limit(1);
      ...
      if (send.broadcastId) {
        const [broadcast] = await db.select().from(broadcasts).where(eq(broadcasts.id, send.broadcastId)).limit(1);
        if (broadcast) {
          subject = broadcast.subject;
          if (broadcast.templateId) {
            const [tmpl] = await db.select().from(emailTemplates).where(eq(emailTemplates.id, broadcast.templateId)).limit(1);
        ...
        const [step] = await db.select().from(campaignSteps).where(eq(campaignSteps.id, send.campaignStepId)).limit(1);
        if (step) {
          const config = step.config as { templateId?: string; subject?: string; htmlContent?: string };
          subject = config.subject ?? subject;
          if (config.templateId) {
            const [tmpl] = await db.select().from(emailTemplates).where(eq(emailTemplates.id, config.templateId)).limit(1);
