...
<!-- Cross-enrollment timeline UI + Time-travel debugging UI shipped 2026-04-30. See ROADMAP "timeline-ui-toolkit" Stable entry. The time-travel mode lives at /campaigns/$id/enrollments/$enrollmentId — scrubber across event_seq + side-by-side StateSnapshot/StateDelta panes + surrounding-events strip with click-to-jump. Uses the SAME pure-function dispatcher (scripts/lib/replay-event-dispatch.ts) as the CLI replay tool — bit-exact forensics parity. -->
